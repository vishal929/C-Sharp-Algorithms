...
namespace DivideAndConquer
{
    public class StrassenMatrixMultiplication
    {
        ...
        public static int[,] StrassenMatrixMultiply(int[,] matrix, int[,] otherMatrix)
        {
            // nxm * m x p = nxp matrix
            // see if dims need to be padded or not

            int resultRows = matrix.GetLength(0);
            int resultCols = otherMatrix.GetLength(1);

            // find the nearest power of 2 greater than or equal to the largest of n,m,p and we will pad all dims to that power of 2
            int largestDim = new List<int> { matrix.GetLength(0), matrix.GetLength(1), otherMatrix.GetLength(0), otherMatrix.GetLength(1) }.Max();

            int low = 1;
            while (low < largestDim)
            {
                low <<=1;
            }
            // now low is the largest power of 2 greater than or equal to the largest dim, we pad the arrays now
            matrix = PadMatrix(matrix, low, low);
            otherMatrix = PadMatrix(otherMatrix, low, low);

            // compute the matrix multiplication
            int[,] product = StrassenMatrixMultiplyInternal(matrix, (0, low - 1, 0, low - 1), otherMatrix, (0, low - 1, 0, low - 1), leafBound:256);

            // extract the product dims from the padded product
            return ExtractMatrix(product, resultRows, resultCols);
        }

        private static int[,] PadMatrix(int[,] matrix, int rowDim, int colDim)
        {
            int[,] padded = new int[rowDim, colDim];
        ...
        private static int[,] ExtractMatrix(int[,] matrix, int rowDim, int colDim)
        {
            int[,] extracted = new int[rowDim, colDim];
            for (int i = 0; i < rowDim; i++)
            {
                for (int j = 0; j < colDim; j++)
                {
                    extracted[i, j] = matrix[i, j];
                }
            }
            return extracted;
        }
        ...
        private static int[,] StrassenMatrixMultiplyInternal(int[,] matrix, (int rowStart,int rowEnd,int colStart,int colEnd) bounds, int[,] otherMatrix, (int rowStart,int rowEnd,int colStart,int colEnd) otherBounds, int leafBound = 64)
        {
            // if we have a certain matrix size, just use regular matrix multiply
            ...
            var P1 = StrassenMatrixMultiplyInternal(matrix, m_1_1, S1, newMatrixBounds);
            var P2 = StrassenMatrixMultiplyInternal(S2, newMatrixBounds, otherMatrix, otherM_2_2);
            var P3 = StrassenMatrixMultiplyInternal(S3, newMatrixBounds, otherMatrix, otherM_1_1);
            var P4 = StrassenMatrixMultiplyInternal(matrix, m_2_2, S4, newMatrixBounds);
            var P5 = StrassenMatrixMultiplyInternal(S5, newMatrixBounds, S6, newMatrixBounds);
            var P6 = StrassenMatrixMultiplyInternal(S7, newMatrixBounds, S8, newMatrixBounds);
            var P7 = StrassenMatrixMultiplyInternal(S9, newMatrixBounds, S10, newMatrixBounds);
